...
![](/assets/images/flareon/7/4.png)

Ở gần cuối của đoạn chat có 1 đoạn nói chuyện khá thú vị

![](/assets/images/flareon/7/5.png)
...
Đoạn `shellcode` này không dài quá, nhưng mình lười đọc nên mình làm như sau: breakpoint tại `jmp eax`, rồi chạy `shellcode`, log lại những hàm và tham số được gọi để có cái nhìn tổng quát về chương trình.
...
![](/assets/images/flareon/7/10.png)
...
![](/assets/images/flareon/7/11.png)

Sau khi nhận `0x4D7` byte, thì dữ liệu đó được đưa vào trong hàm `decr` ở `0x10000143`. Ta thử xem hàm `decr` này có gì.

![](/assets/images/flareon/7/12.png)
...
>
> Trong đoạn code trên, dòng `asm` thứ 2,3,4 chính là pattern nói ở trên.

Key ở lần `RC4` này là `killervulture123` , được lấy ở thanh ghi `esi` (hình ở dưới):

![](/assets/images/flareon/7/14.png)
...
![](/assets/images/flareon/7/15.png)

Đầu tiên là `CreateFile` `C:\\accounts.txt` với `GENERIC_READ`. Ta cũng fake file `C:\accounts.txt` để program có thể đọc.

![](/assets/images/flareon/7/16.png)

Tiếp theo, `ReadFile` để đọc `0x100` bytes từ file này.

Sau đó đoạn `data` vừa read được lại được đưa vào 1 hàm khác để mã hoá, và hàm này rất quen thuộc:

![](/assets/images/flareon/7/17.png)
...
![](/assets/images/flareon/7/19.png)

Như vậy, nội dung của file sẽ được `RC4 encrypt` trước khi gửi về port `1337` cho attacker.

Vậy việc cuối cùng cần làm là, lấy data được gửi trong `wireshark` ra, decrypt và lấy flag.
